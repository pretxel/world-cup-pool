// Canonical source of truth for the daily-quiz seed content (English + ES/FR).
...
//
// English stays canonical in the base prompt/options columns; `es`/`fr` carry
// positionally-aligned translations so the stored correct_index grades every
// locale identically. Option order is NEVER changed across locales.
...
/** Build the translations object in a FIXED key order (es then fr). */
...
--
-- The translations column carries positionally-aligned es/fr translations of
...
// --- emit backfill migration ----------------------------------------------

const migrationHeader = `-- ===========================================================================
-- Daily Quiz — backfill es/fr translations onto already-seeded questions.
-- The seed (supabase/seed/quiz.sql) inserts these for fresh installs, but it is
-- "on conflict (active_on) do nothing", so databases seeded before translations
-- existed keep empty {} translations. This sets each question's translations by
-- its stable active_on key. Idempotent: re-running writes identical JSON, and
-- only the seeded active_on dates are touched (admin-authored rows untouched).
-- GENERATED from scripts/gen-quiz-translations.mjs — edit there, then re-run.
-- ===========================================================================
...
// Canonical daily-quiz seed content (English base + es/fr translations), used by
// tests to assert translation completeness and to drift-check the SQL files.
...
writeFileSync(
  join(ROOT, "supabase/migrations/20260614020000_quiz_question_translations_backfill.sql"),
  migrationSql,
);
...
console.log("  supabase/migrations/20260614020000_quiz_question_translations_backfill.sql");
