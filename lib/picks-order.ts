...
// values become +Infinity so they sort last instead of poisoning the order.
...
// ascending (earliest first). Ties (same kickoff) break by `match_id`
// ascending so the order is total and reproducible run-to-run. Does not mutate
// the input.
export function sortPicksByKickoff<T extends SortablePick>(
  picks: readonly T[],
): T[] {
