// Pure, dependency-free renderer for the daily prediction reminder email.
// Mirrors the quiz/result email visual language (pitch-green header, cream body,
// mono uppercase labels) using email-safe HTML: table layout, inline styles,
// fixed hex colors (no oklch, CSS variables, or stylesheets).
//
// No database or network access — callers assemble `PredictionReminderEmailData`
...
// strings (subject, heading) arrive already interpolated. `vs` is the separator
// rendered between the two teams of each fixture (e.g. "vs").
export interface PredictionReminderEmailStrings {
  subject: string;
  preheader: string;
  eyebrow: string;
  heading: string;
...
// One fixture the player still needs to predict today. `kickoffLabel` is a
...
export interface PredictionReminderEmailData {
  strings: PredictionReminderEmailStrings;
  matches: PredictionReminderMatch[];
  predictionsUrl: string;
  unsubscribeUrl: string;
}

export interface PredictionReminderEmailRendered {
  subject: string;
  html: string;
  text: string;
}
...
function renderBody(s: PredictionReminderEmailStrings): string {
  return `
    <tr>
...
function renderMatches(data: PredictionReminderEmailData): string {
...
function renderCta(data: PredictionReminderEmailData): string {
  return `
    <tr>
...
function renderFooter(data: PredictionReminderEmailData): string {
  const s = data.strings;
  return `
    <tr>
...
export function renderPredictionReminderEmail(
  data: PredictionReminderEmailData,
): PredictionReminderEmailRendered {
  const s = data.strings;
...
function renderText(data: PredictionReminderEmailData): string {
  const s = data.strings;
  const lines: string[] = [];
