...
//
// No database or network access — callers assemble `GroupInviteEmailData` and
...
// inviter and group names.
export interface GroupInviteEmailStrings {
  subject: string;
  preheader: string;
  eyebrow: string;
  heading: string;
...
export interface GroupInviteEmailData {
  ...
  groupName: string;
  joinUrl: string;
  strings: GroupInviteEmailStrings;
}

export interface GroupInviteEmailRendered {
  subject: string;
  html: string;
  text: string;
}
...
function renderBody(data: GroupInviteEmailData): string {
  ...
  return `
    ...
    <tr>
      <td style="padding:18px 28px 8px 28px;">
        <a href="${escapeHtml(data.joinUrl)}" style="display:inline-block;padding:12px 22px;border-radius:10px;background-color:${C.pitch};font-family:${SANS};font-size:15px;font-weight:700;color:${C.pitchFg};text-decoration:none;">${escapeHtml(
          s.joinCta,
        )}</a>
        <p style="margin:12px 0 0 0;font-family:${MONO};font-size:11px;line-height:1.5;color:${C.muted};word-break:break-all;">${escapeHtml(
          data.joinUrl,
        )}</p>
      </td>
    </tr>`;
...
function renderFooter(s: GroupInviteEmailStrings): string {
  return `
    <tr>
...
function renderText(data: GroupInviteEmailData): string {
  const s = data.strings;
  const lines: string[] = [];
  ...
  lines.push(s.intro);
  lines.push("");
  lines.push(`${s.joinCta}: ${data.joinUrl}`);
  lines.push("");
  lines.push(s.footer);
  return lines.join("\n");
}
