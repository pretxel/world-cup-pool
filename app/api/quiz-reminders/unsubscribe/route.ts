...
// One-click unsubscribe for daily quiz reminders. Reached from the email's
...
// friendly confirmation regardless of whether the token matched.

const CONFIRMATION =
  "You've been unsubscribed from daily quiz reminders. You can still play the quiz anytime from the app.";
...
async function optOut(token: string | null): Promise<void> {
  ...
  try {
    ...
    const { error } = await admin
      .from("profiles")
      .update({ quiz_reminder_opt_out: true, email_prefs })
      .eq("unsubscribe_token", token);
    if (error) {
      console.error("[quiz-reminders:unsubscribe] update failed:", error.message);
    }
  } catch (err) {
    console.error("[quiz-reminders:unsubscribe] threw:", err);
  }
}
