...
export default async function AdminMatchesPage({
  ...
  // Active tab is URL-owned (`?tab=`). When an action just redirected back with
  // its sync/confirm result params, default to the Sync tab so the outcome is
  // visible; `reveal` is only valid when knockout rounds exist.
  const activeTab = parseMatchesTab(sp.tab, {
    hasKnockout: knockoutStages.length > 0,
    hasResultParams: syncSummary !== null || confirmUpdated !== null,
  });
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10">
      ...
      <div className="admin-reveal space-y-8">
        ...
        <AdminMatchesTabs
          value={activeTab}
          showReveal={knockoutStages.length > 0}
          labels={{
            fixtures: t("tabFixtures"),
            sync: t("tabSync"),
            reveal: t("tabReveal"),
          }}
          sync={
