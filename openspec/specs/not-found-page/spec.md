...
The not-found (404) experience for both the root not-found page (`app/not-found.tsx`) and the localized not-found page (`app/[locale]/not-found.tsx`). Beyond the `404` headline, body, and recovery links, the pages display a self-hosted, decorative football image that reinforces the product's identity without depending on an external network request, introducing layout shift, or requiring new translation strings.
