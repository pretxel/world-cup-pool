...
question_id)` so that a user receives at most one reminder per question (and thus
per UTC day). A ledger row SHALL be written only after the corresponding email
send succeeds. Re-running the dispatch for the same day SHALL NOT re-email users
already recorded in the ledger.

#### Scenario: Re-run does not duplicate

- **WHEN** dispatch has already emailed a user for today's question and dispatch runs again the same day
