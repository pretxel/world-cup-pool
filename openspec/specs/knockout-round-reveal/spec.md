# knockout-round-reveal Specification

## Purpose

Lets an admin reveal individual knockout rounds for the managed competition so their fixtures appear on the public matches list — as a read-only schedule (date, venue, placeholder participants) — before the teams are confirmed. The reveal flag is stored per knockout stage in the competition format and defaults to hidden. Revealing a round never changes pickability: a fixture stays unpickable until its teams are confirmed (both real countries), it is `scheduled`, and its kickoff is in the future.
