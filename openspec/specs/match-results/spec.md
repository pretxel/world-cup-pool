...
Defines how an admin records a match result and how `public.scores` is rebuilt for every existing prediction against that match. The DB row-change trigger does most of the work, but the server action is responsible for guaranteeing the recompute even on no-op saves and for invalidating the right Next.js caches so users see fresh points immediately.
