# recap-reactions Specification

## Purpose

Turn passive recap-comic views into a social touchpoint by letting signed-in players add lightweight emoji reactions to a match's active recap and by surfacing public, anonymous-readable aggregate counts as social proof — on the match detail page (full interactive bar) and on the landing gallery (summed count). Reactions reuse the platform's RLS, public-read-counts, optional Realtime, and analytics primitives, are bounded by abuse guardrails (uniqueness, allowlist, rate limit, active/final scoping), and carry zero competitive weight.
