# realtime-leaderboard Specification

## Purpose

A live-updating global leaderboard: the public `/leaderboard` page subscribes to Supabase Realtime change events on the `public.scores` base table and re-fetches `v_leaderboard_overall` to keep the on-screen standings current during matchdays without a page reload. The SSR snapshot remains the first paint and the realtime layer is additive, degrading gracefully if the channel never connects.
