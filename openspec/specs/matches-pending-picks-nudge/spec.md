# matches-pending-picks-nudge

## Purpose

A dismissible in-app banner on `/matches` that nudges a signed-in user at the point of highest engagement: it tells them how many upcoming fixtures still need a pick and links them straight to the existing needs-pick filter. It reuses the `needsPickCount` the page already computes (no extra query), is suppressed when nothing is pending, when the filter is already active, and for anonymous visitors, and stays hidden for the rest of the session once dismissed — nudging once without nagging.
