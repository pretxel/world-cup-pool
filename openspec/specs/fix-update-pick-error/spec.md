# fix-update-pick-error Specification

## Purpose

Guarantees that `"use server"` modules export only async functions (server actions) and types, so a non-function export cannot break pages that import them — and that submitting/updating a pick works without the invalid-use-server runtime error.
