...
A first-pick lead state on the public `/matches` page for a signed-in user with no picks and no active filters. It surfaces the soonest still-pickable match with a "Make your first pick" call-to-action (or an encouraging message when nothing is currently pickable), localized, without replacing the fixture list — cutting time-to-first-prediction for brand-new users.
