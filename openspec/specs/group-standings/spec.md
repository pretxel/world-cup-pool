...
The system SHALL expose a public, locale-aware `/standings` page that renders either group or league standings for the active competition and is reachable without authentication. When the active competition has a `league`-kind stage, the page SHALL render a single league table instead of a group table grid. The primary navigation SHALL include a link to this page.
