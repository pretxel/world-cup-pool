# auto-recap-on-final Specification

## Purpose

When a match goes final, the AI recap summary is already generated automatically by the `sync-matches` cron. This capability extends that automation to the remaining two stages of the recap image pipeline — the image prompt and the Leonardo image render — so a completed comic appears with no admin action. Two idempotent, isolated, key-gated batch passes (mirroring the summary pass) run off the same cron, right after the summary pass, chaining summary → image prompt → render within a single run; any stage not reached is picked up by a subsequent run because the passes are convergent, not one-shot. The render completes asynchronously through the existing callback-image webhook, lighting up the landing recap gallery, the recap-digest email, and recap reactions as renders complete.
