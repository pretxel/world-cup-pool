# email-sender-prod-guard

## Purpose

A runtime guard that detects and surfaces production email-sender misconfiguration — the Resend sandbox `EMAIL_FROM` default (`onboarding@resend.dev`) and/or a missing `RESEND_API_KEY` — via a clear warning and a dispatch-summary flag, so degraded deliverability is observable in cron logs and the operations record instead of failing silently. The guard is detection-only: it never throws, mutates env, changes the resolved sender, or alters the existing graceful no-op behavior.
