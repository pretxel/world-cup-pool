...
Defines when an authenticated user may insert or update a prediction. The lock combines two conditions: the target match's `status` must be `scheduled`, AND `kickoff_at` must be in the future. Either condition failing makes the prediction immutable. Enforcement lives at the database row-level-security layer (source of truth) and is mirrored in the server action and UI for clear messaging.
