...
Rules governing the match-detail live feed: a `match_events` timeline model, a per-match live API, opportunistic per-match sync, the shared client polling utility, and the accessible/locale-aware feed UI. Defines how an in-progress match's score and play-by-play events surface to viewers and refresh without a reload.
