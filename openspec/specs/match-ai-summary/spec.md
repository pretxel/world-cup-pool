...
The system SHALL generate a match summary only after a match transitions to `status = 'final'`, using that match's `match_events` timeline ordered by `sequence` plus the final score and team names as the generation input. Generation SHALL NOT run for matches whose status is `scheduled`, `live`, or `cancelled`. If a match reaches `final` with no ingested events, the system MAY still generate a summary from the score and teams alone.

#### Scenario: Final match triggers generation
- **WHEN** a match transitions to `final` and has no existing summary
