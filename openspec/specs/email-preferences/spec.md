# email-preferences Specification

## Purpose

In-app per-type email preferences: a player can independently turn the prediction-reminder, result, and quiz-reminder emails on or off from the account menu, backed by `profiles.email_prefs`, and every email dispatch path honors those choices. The footer one-click unsubscribe stays consistent with the in-app toggles, and the toggle UI is localized.
