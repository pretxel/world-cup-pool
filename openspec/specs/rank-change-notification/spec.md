# rank-change-notification Specification

## Purpose

Surface a player's overall-leaderboard rank delta (direction, magnitude, previous rank) in the post-result email after scores recompute, driven by a pre-recompute rank snapshot. The result email already shows a player's new absolute rank; this capability adds the missing movement hook ("you moved up 3 to #7") by capturing each ranked player's rank before the sync-matches run recomputes scores, then computing the delta against the new rank at email-dispatch time. It degrades gracefully when there is no prior snapshot (first appearance, admin force-resend) or when the rank is unchanged.
