...
Social sharing of a user's match prediction: share actions on the match detail page, the public share landing page that renders a pick from URL parameters, and the dynamically generated Open Graph card image. Exists as a growth loop — a shared pick is both bragging and an invitation. Platform constraints shape the design: Facebook renders only the shared URL's OG card, and Instagram is reachable only through the OS share sheet (`navigator.share`).
## Requirements
