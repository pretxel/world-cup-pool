...
Provide a scheduled, once-per-day, at-most-once email that reminds each opted-in player of today's confirmed, still-open matches they have not yet predicted. The reminder lists each pending match (teams and kickoff time), deep links to the player's picks, and carries a one-click unsubscribe link so players can opt out of prediction reminders independently of any other email, without signing in.
