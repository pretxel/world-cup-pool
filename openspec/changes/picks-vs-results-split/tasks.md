...
- [ ] 1.1 Add `lib/standing-summary.ts` exporting a `getStandingSummary(userId)`
      ...
      `rank`, `rankDelta | null`).
- [ ] 1.2 Source totals from `scores` (points, exact count) and total picks from
      `predictions`, reusing the existing query shape on `/my-picks`.
- [ ] 1.3 Read current rank from `v_leaderboard_overall` (row by `user_id`) and
      previous rank from `leaderboard_rank_snapshot`; compute
      `rankDelta = previousRank - currentRank`, or `null` when no snapshot row
      exists.
- [ ] 1.4 Gate the finals accuracy breakdown on `status = "final"` with both
      scores present and a matching `scores` row, so it agrees with per-match
      badges.
- [ ] 1.5 Unit-test the pure summary derivation (delta sign, missing-snapshot
      null, finals-only gating) with no DB.

## 2. Standing cards component

- [ ] 2.1 Add `components/standing-cards.tsx` — presentational, mobile-first,
...
- [ ] 2.2 Add empty/partial states ("results appear as matches finish",
      no-delta) reusing existing card/empty-state styling.
- [ ] 2.3 Add a client `StandingCardsTracker` that fires `standing_cards_viewed`
      via the existing `trackEvent`, mirroring `LeaderboardViewTracker`.

## 3. My Picks split view

- [ ] 3.1 Render the standing cards at the top of
      `app/[locale]/(app)/my-picks/page.tsx` using `getStandingSummary`.
- [ ] 3.2 Strengthen the per-match rows into a pick-vs-result comparison: pick
      scoreline beside real `home_score–away_score` for `final` matches, with a
      pending state otherwise (no `0–0`, no phantom points).
- [ ] 3.3 Add `components/picks-vs-results.tsx` (or extend the existing groups
...
- [ ] 3.4 Hide the group split when `hasGroupStage` is false; keep the empty
      states for partially-played tournaments.
- [ ] 3.5 Add a client `PicksVsResultsTracker` firing `picks_vs_results_viewed`.

## 4. Signed-in landing standing cards

- [ ] 4.1 In `app/[locale]/page.tsx`, resolve `supabase.auth.getUser()`; for
...
- [ ] 4.2 Leave the anonymous render path untouched (no authed query, demo
      scoreboard unchanged) to preserve SEO/perf.

## 5. Localization

- [ ] 5.1 Add all new keys (split labels: "my picks" / "real results" / pending;
      card labels; finals-accuracy labels; empty states) under the `myPicks` and
      a new `standingCards` namespace in `messages/en.json`.
- [ ] 5.2 Mirror every key in `messages/es.json`, `messages/fr.json`,
      `messages/de.json`.

## 6. Verification

- [ ] 6.1 Confirm no schema/RPC/realtime/cron/push change was introduced (search
      `supabase/migrations` is untouched by this change).
- [ ] 6.2 Verify the split and cards are read-only: no writes to `scores`,
      `predictions`, `leaderboard_rank_snapshot`; no scoring recompute.
- [ ] 6.3 Manually verify across `en/es/fr/de`: no missing-key fallbacks; empty,
      partial, and full-tournament states; signed-in vs. anonymous landing.
- [ ] 6.4 Verify responsive/mobile-first layout and accessibility (semantic
      headings, table headers, contrast) for cards and the two-column split.
- [ ] 6.5 Run lint/typecheck and the standing-summary unit tests; run
