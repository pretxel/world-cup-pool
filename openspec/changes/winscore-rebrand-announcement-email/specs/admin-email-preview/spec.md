...
The admin Operations Emails tab SHALL offer a Previews mode listing every transactional email type (welcome, result, results digest, recap digest, prediction reminder, quiz reminder, playoff score, comeback, score rules, group invite, magic link, winners, announcement). Selecting a type SHALL render the real template function with representative sample data and display the rendered subject, preheader, and HTML body.
...
- **THEN** all thirteen transactional email types are listed and each one renders without error
