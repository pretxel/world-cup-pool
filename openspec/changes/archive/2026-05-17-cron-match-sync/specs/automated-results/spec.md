...
The repository SHALL include a `vercel.json` declaring one cron entry pointing at `/api/cron/sync-matches`. The default schedule SHALL be `*/10 * * * *` (every 10 minutes), with the proposal documenting that this requires the Vercel Pro plan.
