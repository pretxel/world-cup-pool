...
- **No previous-day baseline on first run.** The very first cron run has no `leaderboard_rank_daily` for "yesterday", so rank deltas and the movers section are omitted that day; they appear from the second run onward. Acceptable — the digest is still useful (top 5 + your rank) and self-heals after one day.
