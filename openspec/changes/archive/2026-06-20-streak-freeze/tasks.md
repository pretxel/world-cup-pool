...
- [x] 1.1 **Migration:** add `supabase/migrations/<ts>_streak_freezes.sql`
...
- [x] 1.2 Add RLS: owner-only `select` (`user_id = auth.uid()`); no direct client
      insert/update/delete of grant or consumption rows.
- [x] 1.3 Add a `security definer` RPC (or trusted server path) for the lazy
      weekly grant and the idempotent consumption insert, so freezes can only be
      minted/consumed server-side.
- [x] 1.4 Regenerate Supabase types and add the table/RPC to `lib/db` typings.

## 2. Pure streak functions (freeze-aware)

- [x] 2.1 Extend `computeStreak` in `lib/quiz.ts` to accept an optional
...
- [x] 2.2 Extend `computePredictionStreak` in `lib/prediction-streak.ts` the same
      way, preserving the Monday-anchored weekly window and 7-day cap.
- [x] 2.3 Unit tests: forgiven single gap bridges the streak; a two-day gap with
...
- [x] 3.1 Add `lib/streak-freeze.ts`: the weekly-amount constant, the shared
...
- [x] 3.2 Implement on-read consumption: ensure the current week's grant exists,
...
- [x] 3.3 Unit/integration tests for the helper: eligible gap consumes once and is
...
- [x] 4.1 `/my-picks` (`app/[locale]/(app)/my-picks/page.tsx`): call the freeze
      ...
      used this week.
- [x] 4.2 `/quiz` (`app/[locale]/(public)/quiz/page.tsx`): same for the `quiz`
      kind beside the quiz streak `Stat` (signed-in only).
- [x] 4.3 Add localized copy to `messages/{en,es,fr,de}.json` for the
...
- [x] 5.1 Quiz reminder email (`lib/notifications/quiz-reminder-emails.ts`):
...
- [x] 5.2 Confirm no competitive-scoring impact: no writes to `scores`, no change
      to leaderboards/segmented RPCs/rank snapshots from any freeze code path.
- [x] 5.3 Run `openspec validate "streak-freeze"`; run lint, typecheck, and the
