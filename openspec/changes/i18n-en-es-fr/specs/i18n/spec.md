...
The system SHALL render a language switcher in the global header that lets the visitor choose one of the three supported locales. Selecting a locale SHALL update the `NEXT_LOCALE` cookie and navigate to the same logical route under the new locale prefix.

#### Scenario: Switching from English to Spanish
- **WHEN** a visitor on `/en/matches/<id>` selects "Español" from the switcher
- **THEN** the cookie is set to `NEXT_LOCALE=es`
- **AND** the visitor lands on `/es/matches/<id>` with Spanish content

#### Scenario: Switcher shows all three locales in their own language
- **WHEN** the switcher is open
- **THEN** the three options visible are labelled "English", "Español", "Français"
